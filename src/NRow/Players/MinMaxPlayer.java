...
public class MinMaxPlayer extends PlayerController {
    ...
    public int makeMove(Board board) {
        ...
        actualMove = miniMax(boardTree.getRoot());
...
//        heuristic.getBestAction(playerId, board); // Very useful helper function!
    ...
    public Integer miniMax(Node node) {
        int value;
        int action = 0;
        if (isMaximumPlayer) {
            value = node.getValue();
            for(Node child: node.getChildren()) {
                int newValue = miniMax(child);
                if (newValue > value) {
                    value = newValue;
    ...
    public void constructTree(Integer depth, Board board) {
        ...
        for (int i = 0; i < board.width; i++) {
            if (board.isValid(i)) {
                ...
                newChild.setValue(heuristic.evaluateBoard(playerId, newBoard));
